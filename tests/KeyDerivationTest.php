...
final class KeyDerivationTest extends TestCase
{
    ...
    public function testSecureClear(): void
    {
        ...
        $clearedData = KeyDerivation::secureClear($sensitiveData);

        // 验证返回值是空字符串，表示数据已被清理
        $this->assertEquals('', $clearedData);
        // 验证原始变量未被修改（新的函数行为）
        $this->assertEquals($originalData, $sensitiveData);
        // 验证函数能够处理敏感数据（通过返回值确认）
        $this->assertNotEquals($originalData, $clearedData);
    }
