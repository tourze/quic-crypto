...
class AES128GCM implements AEADInterface
{
    ...
    public function __construct(
        ...
        if (self::KEY_LENGTH !== strlen($key)) {
            throw CryptoException::invalidKey(sprintf('AES-128-GCM 密钥长度必须为 %d 字节，实际为 %d 字节', self::KEY_LENGTH, strlen($key)));
        }
    ...
    private function validateNonce(string $nonce): void
    {
        if (self::NONCE_LENGTH !== strlen($nonce)) {
            throw CryptoException::invalidNonce(sprintf('AES-128-GCM 随机数长度必须为 %d 字节，实际为 %d 字节', self::NONCE_LENGTH, strlen($nonce)));
        }
    }
